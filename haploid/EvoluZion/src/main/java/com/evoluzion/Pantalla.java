...
public class Pantalla implements Screen {
	...
	int numero = 0; // es usado para calculos
	...
	private ShapeRenderer frontera;
	private CheckBox cb_verDatos;
	private ShapeRenderer pausaCaja;
	private TextButton b_stop;
	private TextButton b_ordenar;
	...
	public Pantalla(Evoluzion ev, Mundo m) {
		...
		if (m.verFrontera == true) {
			verFrontera = -1;
		}// determina si se ve o no la frontera en pantalla
		...
		if (verPanel == 1) {
			...
			if (m.verFrontera == false) {
				...
				if (cb_verDatos.isChecked()) {
					...
					fuente.draw(batch, tx.Organismos + m.aorg.size, 5,
							m.alto - 60);
					fuente.draw(batch, tx.mutantes + m.aEspesies.size, 5,
							m.alto - 80);
					int masatotal = (int) (m.BiomasaTotal(m.aorg) + m
							.MateriaLibre());
					fuente.draw(batch, tx.masaTotal + masatotal, 5,
							m.alto - 100);
					fuente.draw(batch, tx.masa + m.MateriaLibre(), 5,
							m.alto - 120);
					...
					fuente.draw(
							batch,
							tx.velocidadMedia
									+ format.format(m.velocidadMedia(m.aorg)),
							5, m.alto - 160);
					fuente.draw(
							batch,
							tx.tamanoMedi
									+ format.format(m.tamanoMedio(m.aorg)), 5,
							m.alto - 180);
					fuente.draw(
							batch,
							tx.tasaMutacionMedia
									+ format2.format(m.tasaMutMedio(m.aorg)),
							5, m.alto - 200);
					fuente.draw(
							batch,
							tx.vidaMdia
									+ format.format(m.longevidadMedio(m.aorg))
									+ " (s)", 5, m.alto - 220);
					fuente.draw(batch,
							tx.resistensiaATB + m.cantidadResistentes(m.aorg),
							5, m.alto - 240);
					fuente.draw(batch,
					...
					fuente.draw(
							batch,
							tx.temOptimaMedia
									+ format.format(m.temOptimaMedia(m.aorg)),
							5, m.alto - 280);
			...
			if (m.verFrontera == true) {
				...
				if (cb_verDatos.isChecked()) {
					...
					fuente.draw(batch,
							...
							m.alto - 60);
					fuente.draw(batch,
							tx.mutantes + m.numEspI,
							5, m.alto - 80);
					int masatotal = (int) (m.BiomasaTotalI(m.aorg) + m
							.MateriaLibreL());
					fuente.draw(batch, tx.masaTotal + masatotal, 5,
							m.alto - 100);
					fuente.draw(batch, tx.masa + m.MateriaLibreL(), 5,
							m.alto - 120);
					...
					fuente.draw(
							batch,
							tx.velocidadMedia
									+ format.format(m.velocidadMediaI(m.aorg)),
							5, m.alto - 160);
					fuente.draw(
							batch,
							tx.tamanoMedi
									+ format.format(m.tamanoMedioI(m.aorg)), 5,
							m.alto - 180);
					fuente.draw(
							batch,
							tx.tasaMutacionMedia
									+ format2.format(m.tasaMutMedioI(m.aorg)),
							5, m.alto - 200);
					fuente.draw(
							batch,
							tx.vidaMdia
									+ format.format(m.longevidadMedioI(m.aorg))
									+ " (s)", 5, m.alto - 220);
					fuente.draw(batch,
							tx.resistensiaATB + m.cantidadResistentesI(m.aorg),
							5, m.alto - 240);
					fuente.draw(batch,
							tx.temperatura + format.format(m.temperatura), 5,
							m.alto - 260);
					fuente.draw(
							batch,
							tx.temOptimaMedia
									+ format.format(m.temOptimaMediaI(m.aorg)),
							5, m.alto - 280);
				...
				if (cb_verDatos.isChecked()) {
					...
					fuente.draw(batch,
							...
							m.alto - 60);
					fuente.draw(batch,
							tx.mutantes + m.numEspD,
							840, m.alto - 80);
					int masatotalD = (int) (m.BiomasaTotalD(m.aorg) + m
							.MateriaLibreR());
					fuente.draw(batch, tx.masaTotal + masatotalD, 840,
							m.alto - 100);
					fuente.draw(batch, tx.masa + m.MateriaLibreR(), 840,
							m.alto - 120);
					fuente.draw(batch, tx.biomasa + m.BiomasaTotalD(m.aorg),
							840, m.alto - 140);
					fuente.draw(
							batch,
							tx.velocidadMedia
					...
					fuente.draw(
							batch,
							tx.tamanoMedi
									+ format.format(m.tamanoMedioD(m.aorg)),
							840, m.alto - 180);
					fuente.draw(
							batch,
							tx.tasaMutacionMedia
									+ format2.format(m.tasaMutMedioD(m.aorg)),
							840, m.alto - 200);
					fuente.draw(
							batch,
							tx.vidaMdia
									+ format.format(m.longevidadMedioD(m.aorg))
									+ " (s)", 840, m.alto - 220);
					fuente.draw(batch,
		...
		if (stage == null) {
			...
			b_frontera.addListener(new InputListener() {
				...
				public boolean touchDown(InputEvent event, float x, float y,
					...
					m.guardarDatos(); // toma los datos del momento que se creo
										// la frontera
